...
export async function resume() {
  ...
  const stateDir = path.join(cwd, 'company/workspace/state');
  ...
  if (!existsSync(lastActionFile)) {
    printError('Foundation is not set up in this directory.');
    console.log('  Run: ' + kleur.cyan('npx foundation-ai@latest init') + '\n');
    return;
  }
